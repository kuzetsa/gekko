...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  var process = function(err, trades) {
    ...
    var f = parseFloat; // Why is this even here? -- kuzetsa 2014 July 16
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  // BEGIN FUNCTION DEFINITION (from inside a different function)
  var calculate = function (err, data) {
    ...
    var itBroke = false; // assume it's NOT broken until proven otherwise

    if (typeof data === "object") {
      // exchange sent back a data object
      log.debug("API data object returned from getPortfolio()");

      if (typeof data.BTC === "object") {
        // exchange included a BTC object within data object 
        log.debug("data.BTC object returned from getPortfolio()");
      } else if (typeof data.BTC === "undefined") {
        // exchange DID NOT include a valid BTC object
        itBroke = true;
        log.debug("data.BTC undefined state during getPortfolio()");
      } else {
        itBroke = true;
        log.debug("strange API data for BTC, neither object nor undefined during getPortfolio()");
      }

      if (typeof data.NMC === "object") {
        // exchange included a NMC object within data object 
        log.debug("data.NMC object returned from getPortfolio()");
      } else if (typeof data.NMC === "undefined") {
        // exchange DID NOT include a valid NMC object
        itBroke = true;
        log.debug("data.NMC undefined state during getPortfolio()");
      } else {
        itBroke = true;
        log.debug("strange API data for NMC, neither object nor undefined during getPortfolio()");
      }

      if (typeof data.GHS === "object") {
        // exchange included a GHS object within data object 
        log.debug("data.GHS object returned from getPortfolio()");
      } else if (typeof data.GHS === "undefined") {
        // exchange DID NOT include a valid GHS object
        itBroke = true;
        log.debug("data.GHS undefined state during getPortfolio()");
      } else {
        itBroke = true;
        log.debug("strange API data for GHS, neither object nor undefined during getPortfolio()");
      }
      ...
      log.debug("strange API data, neither object nor undefined during getPortfolio()");
      itBroke = true;
    }
    ...
    var currency = parseFloat(data.BTC.available); // FIXME: hardcoded

    if (parseFloat(data.BTC.orders)) { // FIXME: hardcoded
      currency -= parseFloat(data.BTC.orders); // FIXME: hardcoded
    }

    var assets = parseFloat(data.GHS.available); // FIXME: hardcoded

    if (parseFloat(data.GHS.orders)) { // FIXME: hardcoded
      assets -= parseFloat(data.GHS.orders); // FIXME: hardcoded
    }
    
    var portfolio = [];

    portfolio.push({name: 'BTC', amount: currency}); // FIXME: hardcoded
    portfolio.push({name: 'GHS', amount: assets}); // FIXME: hardcoded

    callback(err, portfolio);


  }; // END FUNCTION DEFINITION (from inside a different function)
...
Trader.prototype.getFee = function(callback) {
  // cexio fee still hardcoded as of 2014 July 16
  // TODO: isn't there an API call for this?
  callback(false, 0.002); // 0.2% is a multiply by 0.002 
};
...
Trader.prototype.checkOrder = function(order, callback) {
  var check = function(err, result) {
    ...
    var exists = false;
    // Inline function inside another function O_O
    ...
    callback(err, !exists); // FIXME: syntax is !confusing
  };
